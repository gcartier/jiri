...
(define (jazz:build-jiri descriptor #!key (unit #f) (force? #f))
  (let ((unit-specs jazz:jiri-units))
    (jazz:custom-compile/build unit-specs pre-build: jazz:copy-jiri-files unit: unit force?: force?)
...
(jazz:register-product 'jiri
  build: jazz:build-jiri))
